...
// gemma4 GELU slot-packed twins (2026-08-01 g26 decode dig): the gelu base above was the last
// lone-warp-CTA kernel in the g26 decode chain (grid (n_ff, 8), block 32 — sm_90 caps resident
// 1-warp CTAs at 32/SM, so half the SM's warp slots sit idle while each warp chains nsb serial
// dp4a group loads; nsys 512-step diff: 17.5us/layer, ~30% of DRAM SOL). _j8 packs the 8 slots
// into one CTA (warp j = slot j — the silu _j8 geometry); _j8r2 additionally gives each warp 2
// consecutive rows (aqb/d8 register reuse + a second independent dot chain for in-warp latency
// hiding — the down8 w8r2 move). PER-(o,j) FP ORDER IS BIT-IDENTICAL to the base kernel: each
// warp still owns the full (row, slot) dot with the same g-stride-32 loop and the same
// warp_reduce_sum tree — geometry only, NOT a numeric config.
extern "C" __global__ void moe_gate_up_gelu8_dev_q8_j8(
        const unsigned long long* __restrict__ table, const int* __restrict__ sel,
        const signed char* __restrict__ aq, const float* __restrict__ ad,
        float* __restrict__ act,
        int in_f, int n_ff, int n_expert, int qt_g, int qt_u, long rb_g, long rb_u) {
...
extern "C" __global__ void moe_gate_up_gelu8_dev_q8_j8r2(
        const unsigned long long* __restrict__ table, const int* __restrict__ sel,
        const signed char* __restrict__ aq, const float* __restrict__ ad,
        float* __restrict__ act,
        int in_f, int n_ff, int n_expert, int qt_g, int qt_u, long rb_g, long rb_u) {
    ...
    for (int g = lane; g < nsb; g += 32) {
        ...
        for (int r = 0; r < 2; r++) {
            if (r >= rmax) break;
            int o = o0 + r;
            accg[r] += expert_dot_g_v(qt_g, gbase + (long)o * rb_g, g, aqb, d8);
            accu[r] += expert_dot_g_v(qt_u, ubase + (long)o * rb_u, g, aqb, d8);
        }
    }
    #pragma unroll
    for (int r = 0; r < 2; r++) {
        if (r >= rmax) break;
        float x = warp_reduce_sum(accg[r]);
        float au = warp_reduce_sum(accu[r]);
        if (lane == 0) {
            float th = tanhf(0.79788456080286535587989211986876f * x * (1.0f + 0.044715f * x * x));
            act[(size_t)j * n_ff + (o0 + r)] = 0.5f * x * (1.0f + th) * au;
        }
    }
}

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n_new: usize = std::env::var("MEMRA_NGEN").ok().and_then(|v| v.parse().ok()).unwrap_or(256);
    let turn_t: usize = std::env::var("MEMRA_TURN_TOKENS").ok().and_then(|v| v.parse().ok()).unwrap_or(512);

//! M1-PP2 gate (increment 1): the 2-stage pipeline-split eager decode
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cache_pp = memra_engine::cache::Cache::new(&e, &m.cfg, p + n + 8)?;
    ...
    if bad_steps == 0 {
        println!(
            "pp2 gate PASS: {total} steps ({p} prime + {n} gen) BIT-IDENTICAL logits \
             (n_vocab={n_vocab}, n_layers={n_layers}, stage0=[0,{split}), stage1=[{split},{n_layers}))"
        );
        Ok(())
    } else {
        ...
        println!(
            "pp2 gate FAIL: {bad_steps}/{total} steps mismatched (first @ step {s} idx {i}: \
             ref={a:?} pp2={b:?}; split={split}/{n_layers})"
        );
        std::process::exit(1);
    }
}

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(nn) = std::env::var("MEMRA_E4B_GRAPH_GATE") {
        ...
        let model = memra_engine::hybrid::HybridModel::load(&e, &g)?;
        let toks: Vec<u32> = std::env::args().skip(2).filter_map(|s| s.parse().ok()).collect();
        unsafe { std::env::set_var("MEMRA_E4B_GRAPH", "0"); }
        e.stream().synchronize()?;
        let t0 = std::time::Instant::now();
        let a = model.generate(&e, &toks, n)?;
        e.stream().synchronize()?;
        let dt_a = t0.elapsed().as_secs_f64();
        unsafe { std::env::set_var("MEMRA_E4B_GRAPH", "1"); }
        let t1 = std::time::Instant::now();
    ...
    if let (Ok(pdir), Ok(outp)) = (std::env::var("MEMRA_GEN_CORPUS"), std::env::var("MEMRA_GEN_OUT")) {
        let k: usize = std::env::var("MEMRA_SPEC").ok().and_then(|v| v.parse().ok()).unwrap_or(7);
        let dpath = std::env::var("MEMRA_DRAFT").expect("MEMRA_GEN_CORPUS needs MEMRA_DRAFT");
        let n_new: usize = std::env::var("MEMRA_NGEN").ok().and_then(|s| s.parse().ok()).unwrap_or(512);
        let e = memra_engine::Engine::new(0)?;
        let g = memra_gguf::GgufFile::open(&path)?;
        let model = memra_engine::hybrid::HybridModel::load(&e, &g)?;
        let dg = memra_gguf::GgufFile::open(&dpath)?;
        let mut draft = memra_engine::gemma_spec::GemmaDraft::load(&e, &dg)?;
        let tok = memra_tokenizer::Tokenizer::from_gguf(&g).map_err(|e| format!("tokenizer: {e}"))?;
        let eos = tok.eog_ids();
    ...
    if let Ok(dpath) = std::env::var("MEMRA_SPEC_DFLASH") {
        let n_new: usize = std::env::var("MEMRA_NGEN").ok().and_then(|s| s.parse().ok()).unwrap_or(64);
        let e = memra_engine::Engine::new(0)?;
        let g = memra_gguf::GgufFile::open(&path)?;
        let model = memra_engine::hybrid::HybridModel::load(&e, &g)?;
    ...
    if let Ok(kk) = std::env::var("MEMRA_VERIFY_GATE2") {
        ...
        for (i, &tk) in toks.iter().enumerate() {
            ...
            if i >= split { ref_am.push(memra_engine::forward::argmax(&l)); }
        }
        ...
        for (i, seg) in [(0usize, &toks[split..split + k]), (k, &toks[split + k..])] {
            let ams: Vec<usize> = if dev {
                ...
                (0..k).map(|j| memra_engine::forward::argmax(&lv[j * n_vocab..(j + 1) * n_vocab]))
                      .collect()
            };
            for j in 0..k {
    ...
    let n_new: usize = std::env::var("MEMRA_NGEN").ok().and_then(|s| s.parse().ok()).unwrap_or(8);
    let e = Engine::new(0)?;

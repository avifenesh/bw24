...
use memra_gguf::source::{SafetensorsSource, GgufSource, TensorSource};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let lin = (0..cfg.n_layer).find(|&il| cfg.layer_kind(il) == memra_gguf::config::LayerKind::LinearAttention).unwrap();
    let full = (0..cfg.n_layer).find(|&il| cfg.layer_kind(il) == memra_gguf::config::LayerKind::FullAttention).unwrap();

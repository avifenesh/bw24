...
impl GemmaDraft {
    ...
    fn trim_learned_path(&self) -> Option<String> {
        std::env::var("MEMRA_GEMMA_DRAFT_RANKS").ok().map(|p| format!("{p}.learned"))
    }
}
...
impl GemmaDraft {
    pub fn load(e: &Engine, g: &GgufFile) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let swa_pat: Vec<bool> = match g.metadata.get(&format!("{arch}.attention.sliding_window_pattern")) {
            Some(memra_gguf::MetaValue::Array(a)) =>
        ...
        let (head, d2t, trim_adapt) = {
            ...
            match std::env::var("MEMRA_GEMMA_DRAFT_RANKS").ok() {
                Some(path) => {
                    ...
                    let n_spare: usize = std::env::var("MEMRA_GEMMA_TRIM_ADAPT").ok()
...
impl HybridModel {
    ...
    pub fn generate_spec_gemma(&self, e: &Engine, d: &mut GemmaDraft, prompt: &[u32],
        ...
        let adapt_floor_env: Option<usize> = std::env::var("MEMRA_SPEC_ADAPT_FLOOR").ok()
            .and_then(|v| v.parse().ok());
        ...
        let floor_ctx: usize = std::env::var("MEMRA_SPEC_FLOOR_CTX").ok()

...
pub(crate) fn configured_mode() -> SpillIoMode {
    ...
    *MODE.get_or_init(|| {
        ...
        match parse_spill_io(raw.as_deref()) {
            ...
            Err(reason) => {
                eprintln!(
                    "[spill-pread] invalid MEMRA_SPILL_IO={:?} ({reason}); using mmap",
...
pub(crate) fn configured_depth() -> usize {
    ...
    *DEPTH.get_or_init(|| {
        ...
        match parse_depth(raw.as_deref()) {
            ...
            Err(reason) => {
                eprintln!(
                    "[spill-pread] invalid MEMRA_SPILL_PREAD_DEPTH={:?} ({reason}); using 2",
                    raw.as_deref().unwrap_or("")
                );
                2
            }
...
impl WorkerPool {
    fn try_new(depth: usize) -> io::Result<Self> {
        ...
        for worker in 0..depth {
            ...
            let handle = match thread::Builder::new().name(format!("memra-spill-{worker}"))
...
mod tests {
    ...
    fn temp_file(name: &str, bytes: &[u8]) -> (std::path::PathBuf, std::fs::File) {
        let path = std::env::temp_dir().join(format!(
            "memra-pread-{name}-{}", std::process::id()
        ));

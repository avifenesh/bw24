...
fn admit(
    ...
    // Sampled-spec serve: temperature + truncation filters ride the FILTERED rejection-sampling
    // spec path (feat/filtered-spec, transform applied to p and q symmetrically); only penalized
    // requests keep the legacy path (penalty wiring pending).
    let spec = if serve_spec && (sampler.is_greedy() || sampler.is_spec_sampling()) && lm.model.mtp.is_some()
        && seed_fed.is_empty() {
...
fn step_session(
    ...
    if let Some(spec) = s.spec.as_mut() {
        ...
        let sampling = if s.sampler.is_spec_sampling() {

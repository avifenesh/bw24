...
impl HybridModel {
    ...
    fn full_attn_decode_dc_inner(
        ...
        if std::env::var("BW24_NOFA").is_ok() {
            return Err(
                "BW24_NOFA (naive f32 SDPA) is incompatible with the quantized KV cache; \
                        unset BW24_NOFA to use fa_decode_dc"
                    .into(),
            );
        }
    ...
    pub fn generate(
        ...
        // PRIME_MIN_T, BW24_PRIME_TOKENWISE=1, and frozen Hy3 CPU/GPU expert splits take the
    ...
    pub(crate) fn full_attn_decode_pre(
        ...
        if std::env::var("BW24_NOFA").is_ok() {
            return Err(
                "BW24_NOFA (naive f32 SDPA) is incompatible with the quantized KV cache; \
                        unset BW24_NOFA to use fa_decode"
                    .into(),
            );
        }
        e.fa_decode_kvmod(

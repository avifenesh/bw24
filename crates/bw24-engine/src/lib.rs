...
impl Engine {
    ...
    /// persistent launch (all workers co-resident — gridDim sized from the occupancy API,
    /// soft-barriered between dependent segments). Per-row/per-quad programs are the exact
    /// standalone kernels' — bit-identical y_down; kills 2 launch/drain boundaries + the
    /// gelu launch per layer per round. Returns f0 = down output. q4_0 rp x3, m in 2..=8.
    #[allow(clippy::too_many_arguments)]
    pub fn matmul_q4_ffn_slab(&self, wg: &crate::model::GpuTensor, wu: &crate::model::GpuTensor,
                              wd: &crate::model::GpuTensor,
                              zq: &CudaSlice<i8>, zd: &CudaSlice<f32>, m: usize)
        ...
        let (Some(n_ff), Some(o_u), Some(out_down)) = (q4(wg), q4(wu), q4(wd)) else {
            return Ok(None);
        };
        if n_ff != o_u || wg.in_features() != wu.in_features()
            || wd.in_features() != n_ff || n_ff % 128 != 0 {
            return Ok(None);
        }
        fn eff(w: &GpuTensor) -> (&CudaSlice<u8>, bool) {
            match w {
        ...
        let ((bg, rg), (bu, ru), (bd, rd)) = (eff(wg), eff(wu), eff(wd));
        if !rg || !ru || !rd { return Ok(None); }
        let mcols = Self::batched_mcols(m);
        let name = match mcols { 2 => "qmatvec_q4_0_ffn_slab_b2_rp",
                                 4 => "qmatvec_q4_0_ffn_slab_b4_rp",
                                 _ => "qmatvec_q4_0_ffn_slab_b8_rp" };
        let f = self.func(name);
        // co-residency contract: gridDim MUST NOT exceed resident capacity or the soft
        // barriers deadlock. Occupancy queried per (kernel, block) once.
        static NW: std::sync::OnceLock<u32> = std::sync::OnceLock::new();
        let workers = *NW.get_or_init(|| {
            let per_sm = f.occupancy_max_active_blocks_per_multiprocessor(128, 0, None)
                .unwrap_or(1).max(1);
            use cudarc::driver::sys::CUdevice_attribute_enum as A;
            let sms = self.gpu.ctx.attribute(A::CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT)
                .unwrap_or(82) as u32;
            per_sm as u32 * sms
        });
        let in_f = wg.in_features();
        let mut y_gate = self.alloc_uninit::<f32>(m * n_ff)?;
        let mut y_up = self.alloc_uninit::<f32>(m * n_ff)?;
        let mut act_q = self.alloc_i8_uninit(m * n_ff)?;
        let mut act_d = self.alloc_uninit::<f32>(m * (n_ff / 32))?;
        let mut y_down = self.alloc_uninit::<f32>(m * out_down)?;
        let mut ctr = self.gpu.stream.alloc_zeros::<i32>(2)?;
        let (inf, nff, od, mi) = (in_f as i32, n_ff as i32, out_down as i32, m as i32);
        let cfg = LaunchConfig { grid_dim: (workers, 1, 1), block_dim: (32, 4, 1),
                                 shared_mem_bytes: 0 };
        let mut b = self.gpu.stream.launch_builder(&f);
        b.arg(bg).arg(bu).arg(bd).arg(zq).arg(zd)
         .arg(&mut y_gate).arg(&mut y_up).arg(&mut act_q).arg(&mut act_d).arg(&mut y_down)
         .arg(&inf).arg(&nff).arg(&od).arg(&mi).arg(&mut ctr);
        unsafe { b.launch(cfg)?; }
        Ok(Some(y_down))
    }

...
use bw24_gguf::{GgmlType, GgufFile};
    ...
    Ok(
        if let Some(moe) = cfg.moe.as_ref().filter(|_| !dense_override) {
            ...
            let exp_probs_b = src
                ...
                .map(|v| bw24_gguf::dequant::dequantize(v.ggml_type, &v.bytes, n_expert));
...
fn build_dev_exps(
    ...
    if gu_il {
        eprintln!("[moe] gate/up dev slab INTERLEAVED (BW24_MOE_GU_IL)");
    }
...
impl HybridModel {
    ...
    fn load_from_source_impl(
        ...
        let mtp = if load_mtp {
            match std::env::var("BW24_MTP_DRAFT") {
        ...
        {
            eprintln!(
                "[frspec-trim] DISABLED under BW24_FULL_PREC — using the natural full MTP head"
            );
        }
        let mtp = match (
            if crate::model::full_prec_enabled() {
            ...
            (Ok(path), Some(mut head)) if !path.is_empty() => {
                ...
                let d2t_t = tg
                    .find("d2t")
                    .expect("BW24_FRSPEC_TRIM file has no d2t tensor");

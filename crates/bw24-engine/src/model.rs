...
use bw24_gguf::source::{DiskExtent, GgufSource, TensorSource};
...
impl GpuTensor {
    ...
    pub fn load_from_source(
        ...
        let st_direct = std::env::var("BW24_ST_DIRECT")
            .map(|v| v != "0")
            .unwrap_or(true);
        if rp_enabled() && st_direct && !cutlass_wants_raw {
            if let Some(nv) = src.find_nvfp4_native(name) {
                if nv.in_f % 64 == 0 && nv.out_f > 0 {
                    ...
                    let bytes =
                        e.htod_bytes(&bw24_gguf::nvfp4_repack::repack_modelopt_to_split(
        ...
        let kq = std::env::var("BW24_KQ_NVFP4")
            .ok()
                    ...
                    match src.find_fp8_native(name) {
                                ...
                                && f8.out_f as u64 == v.ne[1] =>
                        {
                            ...
                            let budget = *FP8_BUDGET.get_or_init(|| {
                                std::env::var("BW24_PP_FP8_BUDGET_MB")
                                    .ok()
...
impl Model {
    ...
    pub fn load_dense_from_source(
        ...
        assert!(
            ...
            "model has linear-attn layers; use hybrid path"
        );
        // FP8-KV per-model door: OFF everywhere by default (explicit BW24_KV_FP8 wins).
    ...
    pub fn load_from_source(
        ...
        // Per-expert `weight_scale_2` macros go to `macros` (folded post-matmul by the MoE forward).
        {
            ...
            if let Some(nv0) = src.find_nvfp4_native(&name0) {
                ...
                let disk = std::env::var("BW24_ST_REPACK_DISK")
                    .map(|v| v != "0")
                    .unwrap_or(true)
                    && src.st_dir().is_some();
                ...
                let bytes = if disk {
                    ...
                    let tiers = if std::env::var("BW24_ST_PINNED")
                        .map(|v| v == "1")
                        .unwrap_or(false)
                    {
...
mod tests {
    use super::{
        repack_nvfp4_split, unpack_nvfp4_split, ExpertKeepalive, ExpertSource, HostBuf, HostExps,
        QT_BF16, QT_NVFP4, QT_Q2_K, QT_Q4_K,
    };
    ...
    use bw24_gguf::{config::ModelConfig, GgmlType};
    use std::borrow::Cow;
    ...
    fn mixed_expert_loader_keeps_mmap_backing_zero_copy() {
        let path = std::env::temp_dir().join(format!("bw24-mixed-mmap-{}", std::process::id()));
    ...
    fn tiered_expert_source_does_not_double_apply_layout_offset() {
        let path =
            std::env::temp_dir().join(format!("bw24-tiered-source-offset-{}", std::process::id()));
    ...
    fn legacy_mmap_source_requires_retained_file_extent() {
        let path =
            std::env::temp_dir().join(format!("bw24-legacy-mmap-source-{}", std::process::id()));
        let expert_len = 2 * 84;
    ...
    fn uniform_expert_loader_coalesces_contiguous_mmap() {
        let path = std::env::temp_dir().join(format!("bw24-uniform-mmap-{}", std::process::id()));

...
fn worker_prefetch_window() -> usize {
    ...
    *WINDOW.get_or_init(|| {
        ...
        std::env::var("BW24_SPILL_WORKER_EXPERT_WINDOW")
            .ok()
...
fn cpu_expert_profile_admit_enabled() -> bool {
    static E: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *E.get_or_init(|| std::env::var("BW24_CPU_EXPERT_FREEZE_PROFILE_ADMIT").as_deref() == Ok("1"))
}
        ...
        for (il, layer) in self.layers.iter().enumerate() {
            ...
            match &layer.mixer {
                Mixer::Full(fa) => {
                    ...
                    // task #18 (attn side): ONE varlen FA launch for every fresh sequence
                    // (per-seq FA at T~152 runs 3 q-tiles x n_head CTAs — SM-starved).
                    // Same per-block math (bit-gateable); BW24_FA_VL=0 or a non-bf16kv
                    // config falls back to the per-seq dispatch.
                    ...
                    if use_favl {
                        ...
                        for (s, g3s) in parts.into_iter().enumerate() {
                            let (pre, base_len) = self.full_attn_prime_pre_fa(
                                e, fa, g3s, &pos_ds[s], ts[s], caches[s], il)?;
                            assert!(base_len == 0, "prime_cache_batch is fresh-only");
                            pres.push(pre);
                        }
                        ...
                        let fargs: Vec<crate::FaSeqVl> = (0..b).map(|s| crate::FaSeqVl {
                            q: e.addr_f32(&pres[s].q), k16: e.addr_u8(&mirrors[s].0),
                            v16: e.addr_u8(&mirrors[s].1), o: e.addr_f32(&attns[s]),
                            kf: e.addr_f32(&pres[s].k), vf: e.addr_f32(&pres[s].v),
                            t: ts[s] as i32, pad: 0,
                        }).collect();
                        ...
                        for (s, attn) in attns.into_iter().enumerate() {
                            let (attn_g, ag16) = self.full_attn_prime_post_fa(
                                e, attn, &pres[s].gate, ts[s], n_head, head_dim)?;
                            let mut done = false;
                            if let Some(xh) = &ag16 {
...
impl HybridModel {
    ...
    pub(crate) fn moe_ffn_inner(
        ...
        let epoch_lfu = std::env::var_os("BW24_MOE_LFU_DECAY").is_some();
    ...
    pub(crate) fn moe_ffn_sequential_zq8(
        ...
        if cpu_expert_requested && (cfg.hy3.is_none() || cfg.m3.is_some()) {
            return Err(std::io::Error::other(
                "BW24_CPU_EXPERT_LIB is experimental and currently gated to Hy3",
            )
            .into());
        }
        ...
        let freeze_cpu_residency = cpu_expert_requested
            && std::env::var("BW24_CPU_EXPERT_FREEZE_CACHE").as_deref() == Ok("1");
        let caller_warms_before_freeze = std::env::var("BW24_CPU_EXPERT_FREEZE_WARMUP_TOKENS")
            .ok()
            .and_then(|value| value.parse::<usize>().ok())
        ...
        for tok in 0..t {
            ...
            for (j, &ex) in sel.iter().enumerate() {
                ...
                if cache_dispatch && (gate_q8 || up_q8 || down_q8) {
                    // dp4a EXPERT PATH (BW24_MOE_Q8): quantize z-row once per token. Mixed expert

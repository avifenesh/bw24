// f16_prefill.cu — BW24_PP_F16 prefill GEMM: cuBLASLt FP16 TN on a resident fp16 dequant
...
//
// NUMERIC CONFIG (new, explicit, gated — GDN-chunked/BW24_PP_FP8 precedent): weight int8 -> fp16
...
//   bw24_f16_pp_gemm : f32->fp16 activation convert + cublasLtMatmul TN, all on one stream,
//                      (m,n,k)-cached descriptors + heuristic algo under a mutex.
//   bw24_q8_0_dequant_f16 : GGUF Q8_0 34B blocks -> row-major fp16 mirror (load-time, per tensor).
...
extern "C" __global__ void bw24_q8f16_dequant_kernel(const unsigned char* __restrict__ src,
...
extern "C" int bw24_q8_0_dequant_f16(const void* w_q8, void* w_f16, long out_f, long nblk_row,
                                     void* stream_v) {
    cudaStream_t stream = (cudaStream_t)stream_v;
    size_t nblk_total = (size_t)out_f * (size_t)nblk_row;
    int threads = 256;
    size_t blocks = (nblk_total + threads - 1) / threads;
    bw24_q8f16_dequant_kernel<<<(unsigned)blocks, threads, 0, stream>>>(

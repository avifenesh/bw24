...
// ======================================================================================
// K32-IMMA variant (BW24_MMQ_K32=1, lever 2 of the pp ladder): ONE int8 m16n8k32 MMA per
// 8-int step (2x FLOP per issued tensor instruction on the ISSUE-LIMITED tensor pipe —
// ncu 2026-07-10: tensor 59.6% = the wall, IPC 2.07). Requires per-32 weight scales:
// per sub-pair s32 = max(sa,sb); values re-code w' = round(kv * (s_sub/s32) * 8) (int8,
// max |12*8|=96; small-value re-code error ~2% class); epilogue dA = s32/16 (the /2
// kvalues convention x the /8 headroom). NEW NUMERIC CONFIG — full battery decides.
// ======================================================================================
#define MMQ_MMA_TILE_X_K_K32 (2 * MMQ_TILE_NE_K + MMQ_TILE_NE_K / 4 + 4)   // 76, same as f8f4

static __device__ __forceinline__ void bw24_mma_s8_k32(
        int * __restrict__ c, const int * __restrict__ a, const int b0, const int b1) {
...
template <int mmq_y, bool need_check, bool is_rp>
static __device__ __forceinline__ void load_tiles_nvfp4_k32(
        const char * __restrict__ x, const char * __restrict__ x_sc, int * __restrict__ x_tile,
        const int kb0, const int i_max, const int stride) {
    constexpr int nwarps = mmq_y / 16;
    constexpr int warp_size = MMQ_WARP_SIZE;
...
#pragma unroll
        for (int pair = 0; pair < 2; ++pair) {
            ...
            const float sb = ggml_cuda_ue4m3_to_fp32(src_d[2 * pair + 1]);
            const float s32 = fmaxf(fmaxf(sa, sb), 1e-30f);
            x_df[i * MMQ_MMA_TILE_X_K_K32 + 2 * kbx + pair] = s32 * 0.125f;  // /8 headroom fold
#pragma unroll
            for (int half = 0; half < 2; ++half) {
                const int sub = 2 * pair + half;
                const float r8 = ((half == 0 ? sa : sb) / s32) * 8.0f;
                int lut[4];
#pragma unroll
                for (int w = 0; w < 4; ++w) {
                    // kvalues_mxfp4 convention (e2m1 x2, /2 baked into ue4m3_to_fp32)
                    const int v0 = __float2int_rn((float) kvalues_mxfp4[4*w+0] * r8);
                    const int v1 = __float2int_rn((float) kvalues_mxfp4[4*w+1] * r8);
                    const int v2 = __float2int_rn((float) kvalues_mxfp4[4*w+2] * r8);
                    const int v3 = __float2int_rn((float) kvalues_mxfp4[4*w+3] * r8);
                    lut[w] = (v0 & 0xff) | ((v1 & 0xff) << 8) | ((v2 & 0xff) << 16) | ((v3 & 0xff) << 24);
                }
                const int2 q0 = get_int_from_table_16(src_qs[2 * sub + 0], (const int8_t *) lut);
                const int2 q1 = get_int_from_table_16(src_qs[2 * sub + 1], (const int8_t *) lut);
                x_qs[i * MMQ_MMA_TILE_X_K_K32 + kqs + 4 * sub + 0] = q0.x;
                x_qs[i * MMQ_MMA_TILE_X_K_K32 + kqs + 4 * sub + 1] = q1.x;
                x_qs[i * MMQ_MMA_TILE_X_K_K32 + kqs + 4 * sub + 2] = q0.y;
                x_qs[i * MMQ_MMA_TILE_X_K_K32 + kqs + 4 * sub + 3] = q1.y;
            }
        }
    }
}
...
template <int mmq_x, int mmq_y>
static __device__ __forceinline__ void vec_dot_nvfp4_k32_mma(
        const int * __restrict__ x, const int * __restrict__ y, float * __restrict__ sum, const int k00) {
    typedef tile<16, 8, int> tile_A_8;
    typedef tile< 8, 4, int> tile_B;
    typedef tile<16, 8, int> tile_C;
...
#pragma unroll
        for (int k01 = 0; k01 < MMQ_TILE_NE_K; k01 += 8) {
            const int k0 = k00 + k01;
            load_ldmatrix(A[n][k01 / 8], x_qs + (i0 + n * tile_A_8::I) * MMQ_MMA_TILE_X_K_K32 + k0,
                          MMQ_MMA_TILE_X_K_K32);
        }
#pragma unroll
        for (int l = 0; l < tile_C::ne / 2; ++l) {
            const int i = i0 + n * tile_C::I + tile_C::get_i(2 * l);
#pragma unroll
            for (int k01 = 0; k01 < MMQ_TILE_NE_K; k01 += 8) {
                dA[n][l][k01 / 8] = x_df[i * MMQ_MMA_TILE_X_K_K32 + (k00 + k01) / 8];
            }
        }
    }
...
#pragma unroll
            for (int n = 0; n < ntx; ++n) {
                int C[4] = {0, 0, 0, 0};
                bw24_mma_s8_k32(C, A[n][k01 / 8].x, B[0].x[0], B[1].x[0]);
#pragma unroll
                for (int l = 0; l < tile_C::ne; ++l) {
                    sum[(j0 / tile_C::J + n) * tile_C::ne + l] +=
                        dA[n][l / 2][k01 / 8] * dB[l % 2] * (float) C[l];
                }
            }
        }
    }
...
template <int mmq_x, int mmq_y, bool need_check, bool is_rp>
static __device__ __forceinline__ void mul_mat_q_process_tile_nvfp4_k32(
        const char * __restrict__ x, const char * __restrict__ x_sc, const int offset_x,
        const int * __restrict__ y,
        const int * __restrict__ ids_dst, float * __restrict__ dst,
        const int stride_row_x, const int ncols_y, const int stride_col_dst,
    ...
    for (int kb0 = kb0_start; kb0 < kb0_stop; kb0 += blocks_per_iter) {
        load_tiles_nvfp4_k32<mmq_y, need_check, is_rp>(x, x_sc, tile_x, offset_x + kb0, tile_x_max_i, stride_row_x);
        {
            const int * by0 = y + ncols_y * (kb0 * qk / ne_block) * sz;
#pragma unroll
            for (int l0 = 0; l0 < mmq_x * MMQ_TILE_Y_K; l0 += nwarps * warp_size) {
        ...
        __syncthreads();
        vec_dot_nvfp4_k32_mma<mmq_x, mmq_y>(tile_x, tile_y, sum, 0);
        __syncthreads();
        {
            const int * by0 = y + ncols_y * ((kb0 * qk / ne_block) * sz + sz);
#pragma unroll
        ...
        __syncthreads();
        vec_dot_nvfp4_k32_mma<mmq_x, mmq_y>(tile_x, tile_y, sum, MMQ_TILE_NE_K);
        __syncthreads();
    }
...
template <int mmq_x, int mmq_y, bool need_check, bool is_rp>
static __global__ void mul_mat_q_nvfp4_k32(
        const char * __restrict__ x, const char * __restrict__ x_sc,
        const int * __restrict__ y, float * __restrict__ dst,
        const int nrows_x, const int ncols_dst, const int stride_row_x, const int ncols_y,
        const int stride_col_dst, const int blocks_per_ne00, const float out_scale) {
...
#pragma unroll
    ...
    mul_mat_q_process_tile_nvfp4_k32<mmq_x, mmq_y, need_check, is_rp>(
        x, x_sc, offset_x, y + offset_y, ids_dst_shared, dst + offset_dst, stride_row_x, ncols_y,
        stride_col_dst, tile_x_max_i, tile_y_max_j, 0, blocks_per_ne00, out_scale);
}

extern "C" {
int bw24_mmq_nvfp4_k32(const void * W_nvfp4_blocks, const float * act_f32, float * y,
                       int in_f, int out_f, int n_tokens, void * act_scratch, void * stream,
                       float out_scale, int rp) {
    cudaStream_t st = reinterpret_cast<cudaStream_t>(stream);
    // int8 activations — the SAME q8_1 d4 quantizer as the daily W4A8 path.
    const int64_t ne10 = in_f;
    const int64_t ne10_padded = GGML_PAD(ne10, MATRIX_ROW_PADDING);
    {
        const int64_t block_num_y = (ne10_padded + 4 * CUDA_QUANTIZE_BLOCK_SIZE_MMQ - 1) /
        ...
        const dim3 num_blocks((unsigned) n_tokens, (unsigned) block_num_y, 1);
        quantize_mmq_q8_1_d4_kernel<<<num_blocks, block_size, 0, st>>>(
            act_f32, act_scratch, ne10, in_f, ne10_padded, n_tokens);
        cudaError_t e = cudaGetLastError();
        if (e != cudaSuccess) { return 1000 + (int) e; }
    }
    const int stride_row_x    = in_f / QK_NVFP4;
    ...
    const size_t smem = (size_t) MMQ_X * sizeof(int)
                      ...
                      + (size_t) MMQ_Y * MMQ_MMA_TILE_X_K_K32 * sizeof(int);
    const bool need_check = (out_f % MMQ_Y) != 0;
    const int * y_q = (const int *) act_scratch;
    const char * W  = (const char *) W_nvfp4_blocks;
    const char * W_sc = W + (size_t) out_f * (in_f / QK_NVFP4) * 32;
    #define BW24_K32_LAUNCH(NC, RP) do {                                                                 \
        cudaFuncSetAttribute(mul_mat_q_nvfp4_k32<MMQ_X, MMQ_Y, NC, RP>,                                  \
                             cudaFuncAttributeMaxDynamicSharedMemorySize, smem);                         \
        mul_mat_q_nvfp4_k32<MMQ_X, MMQ_Y, NC, RP><<<grid, block, smem, st>>>(                            \
            W, W_sc, y_q, y, out_f, n_tokens, stride_row_x, ncols_y, stride_col_dst, blocks_per_ne00,    \
            out_scale);                                                                                  \
    } while (0)
    if (rp) { if (need_check) { BW24_K32_LAUNCH(true, true);  } else { BW24_K32_LAUNCH(false, true);  } }
    else    { if (need_check) { BW24_K32_LAUNCH(true, false); } else { BW24_K32_LAUNCH(false, false); } }
    #undef BW24_K32_LAUNCH
    cudaError_t e = cudaGetLastError();
    if (e != cudaSuccess) { return 2000 + (int) e; }
    return 0;
}

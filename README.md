...
**Current standing: seven supported models, all fully gated, and no plain cell anywhere below llama.cpp (2026-07-25; the 12B decode cell sits AT parity — it reads 1.00x and straddles it by ±0.4% window to window). Qwen leads on every cell (plain 1.06-1.08x, spec 1.06-2.30x). Gemma leads decisively where llama lacks the capability or the depth (31B spec 1.7k 1.16x, E4B spec ≥1.23x, E4B plain 1.10x), takes 1.14x on the new 12B MTP chat cell, and holds 1.00-1.07x elsewhere under the strictest best-vs-best pairing — both engines' draft depths swept, not just ours (12B bring-up closed 2026-07-24; 12B/31B chat spec cells re-paired 2026-07-25; 26B/E4B re-audit 2026-07-15).** Every number below is a same-session, same-prompt, interleaved measurement against llama.cpp's best config; exactness is gated (argmax match + speculative self-consistency) on every kernel change, so speed never buys different outputs.
...
| MTP spec, chat (K=3 + own-gen trim) | 200.7 | 176.3 | **1.14x** |
...
| 31B MTP spec, chat (K=4 + own-gen trim) | 103.8 | 99.4 | 1.04x |
...
The two chat-cell spec rows (12B and 31B, 2026-07-25) are the first pairs measured with **both** engines swept: bw24 at its K optimum (12B K=3, 31B K=4) against llama at its own `--spec-draft-n-max` optimum (12B 2, 31B 4). That sweep mattered — llama's 12B default of n-max 3 sits 12% below its own best, and pairing against it would have published 1.28x instead of the honest 1.14x. It also revised the 31B chat row down from a previously published 1.06x: llama's side gained more from sweeping than ours did. The 12B drafter trim is this campaign's own artifact (`gemma4-12b-owngen-ranks-32768.gguf.txt`, own-generation ranks over a 254-prompt class-covering corpus with the chat template on) and buys +4.9% e2e over the untrimmed head.

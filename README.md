...
**Current standing: seven supported models, all fully gated, and no plain cell anywhere below llama.cpp (2026-07-26; the 12B decode cell sits AT parity — it reads 1.00x and straddles it by ±0.4% window to window). Qwen leads on every cell (plain 1.06-1.08x, spec 1.06-2.30x). Gemma now leads every spec cell it serves — 12B chat MTP 1.36x, 31B chat MTP 1.26x, 31B 1.7k 1.16x, E4B ≥1.23x — plus E4B plain 1.10x, and holds 1.00-1.07x elsewhere under the strictest best-vs-best pairing: both engines' draft depths swept, re-paired same-session (12B bring-up closed 2026-07-24; 12B/31B chat spec cells 2026-07-26; 26B/E4B re-audit 2026-07-15).** Every number below is a same-session, same-prompt, interleaved measurement against llama.cpp's best config; exactness is gated (argmax match + speculative self-consistency) on every kernel change, so speed never buys different outputs.

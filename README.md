...
Measured 2026-07-10 on the target rig (RTX 5090 Laptop, N=2 medians, engines interleaved per model in the same hour-regime, idle-gated cold starts, no flags (tuned paths are defaults); rebaseline2 campaign + 35B spec refresh 2026-07-10 (router GEMV + CSR gate_up dedup; H-runs, N=2, 1590MHz/58-65C, idle GPU, llama = marked 07-09 floor), FA-v4 refresh 2026-07-10 (J-runs N=2; 35B p1 -0.5% = the v4 shallow-ctx cost, honest); 27B p1/p2 re-paired 2026-07-10 (I-runs: same-session interleaved N=2, idle GPU; p3 sampled pair unchanged), raw logs in research/tune-data/rebaseline-logs/) against llama.cpp built on the same machine, same exact prompts, both engines re-baselined the same day. Boards move with the tuning campaign — `research/tune-data/rig5090.jsonl` is the running record; the README is refreshed with every board-moving merge.
<!-- PERF-DATE:END -->

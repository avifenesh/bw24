...
Every number below is a same-session, same-prompt measurement against llama.cpp's best config on this box. Exactness is gated: argmax match and speculative self-consistency on every kernel change.
...
Columns: short-code / medium-code (greedy temp-0) / long-agentic sampled (temp 0.7, seeded, distribution-exact rejection sampling). Mechanisms behind the spec edge — FR-Spec vocabulary trims, confidence gating, per-content-class draft depth — in [`HANDOVER.md`](HANDOVER.md).
...
- Gemma depth cells above are the open tuning front.

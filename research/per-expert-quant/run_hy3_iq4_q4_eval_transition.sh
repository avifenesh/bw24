...
# Evaluate the pre-frozen six-format arm only after its private build/heal gate and the original
...
BUILD_COMPLETE=${BUILD_COMPLETE:-/data/logs/iq4-q4-extension-99f3dc3/complete}
ART_ROOT=${ART_ROOT:-/scratch/bw24-artifacts-iq4-q4-99f3dc3}
...
OUT_ROOT=${OUT_ROOT:-/data/results/per-expert-quant/iq4-q4-directional-v1}
LOG_ROOT=${LOG_ROOT:-/data/logs/iq4-q4-directional-v1}
CACHE_DIR=${CACHE_DIR:-/data/cache/per-expert-evals}
PRIVATE_GATE_ROOT=${PRIVATE_GATE_ROOT:-/data/logs/iq4-q4-private-artifact-gate-99f3dc3}
...
RUN_ID=iq4-q4-v1-$(git -C "$ROOT" rev-parse --short=7 HEAD)-$(date -u +%Y%m%dT%H%M%SZ)
...
d={"format":"bw24-iq4-q4-directional-run-v1","run_id":run_id,
...
CUDA_VISIBLE_DEVICES=0 taskset -c 0-11 numactl --membind=0 \
  env ARM="$ARM" ARTIFACT="$artifact" RUN_ID="$RUN_ID" SERVER_BIN="$SERVER_BIN" \
    ...
    HOURISH_SCORER_LOCK=/tmp/bw24-iq4-q4-scorer.lock \
...
"$PY" "$EVAL_ROOT/research/per-expert-quant/summarize_hourish_results.py" \
  ...
  --server-sha256 "$SERVER_SHA256" --output "$OUT_ROOT/iq4-q4-summary-$RUN_ID.json"
...
frontier="$OUT_ROOT/iq4-q4-frontier-$RUN_ID.json"
...
PY
sha256sum "$OUT_ROOT/run-configs/$RUN_ID.json" "$OUT_ROOT/iq4-q4-summary-$RUN_ID.json" \

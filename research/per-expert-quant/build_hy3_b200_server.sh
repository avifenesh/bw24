...
TARGET_DIR=${TARGET_DIR:-/data/build/bw24-hy3-b200-target}
...
python3 - "$ROOT/receipts/bw24-server-build.json" "$binary" "$binary_sha" \
  "$(rustc --version)" "$(cargo --version)" "$(git -C "$REPO" rev-parse HEAD)" <<'PY'
import json
import pathlib
import sys
from datetime import datetime, timezone

output, binary, sha, rustc, cargo, git_head = sys.argv[1:]

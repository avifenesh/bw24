...
// -Xptxas --apply-controls lands on the real bw24_fa3_prefill kernel being timed.
//
...
//     -lcuda -o /tmp/acfr tools/acf_fa3_runner.cu crates/bw24-engine/cu/fa3_prefill.cu
...
int main(int argc, char** argv) {
    ...
    int rc = bw24_fa3_prefill(dQ, dK, dV, dO, T, H, HKV, D, scale, nullptr);
    ...
    for (int i = 0; i < 10; i++) bw24_fa3_prefill(dQ, dK, dV, dO, T, H, HKV, D, scale, nullptr);
    CK(cudaDeviceSynchronize());
    CK(cudaEventRecord(a));
    for (int i = 0; i < 50; i++) bw24_fa3_prefill(dQ, dK, dV, dO, T, H, HKV, D, scale, nullptr);

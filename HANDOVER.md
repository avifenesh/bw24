...
NEXT SPILLED-MoE TARGET ASSESSED (2026-07-09, owner-directed): Hy3 REAP75 replaces M3's slot. Base tencent/Hy3 = 298.8B hy_v3 MoE; REAP75 ≈ 75B ≈ 42GB @4-bit = fits VRAM+RAM with NO disk tier (the first spilled-MoE that escapes NVMe on this rig → 10-30 tok/s class plausible vs M3's 1.5). Blockers priced: no CUDA-format checkpoint exists (published cuts are MLX 4-bit only — transcode MLX→Q4_K keeps asymmetry, MLX→NVFP4 pays the asym→sym tax; quality gates decide), and hy_v3 is a new arch port (Hunyuan MoE; M3's sigmoid-router work is the prior). REAP50 (~84GB) stays over capacity = same trap as M3, not a target. 2-bit REAP25 variants exist but 2-bit quality is presumed disqualifying.

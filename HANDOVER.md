...
**GAP 2 (spec degrades with ctx) — ATTACKED 2026-07-03, two stages, nsys-decomposed first (JSONL `spec-longctx-profile`).** The 64-tok steady loop at 9B/6.3k was: 44% MMVQ trunk weights (verify + the partial-accept REPLAY duplicate pass, ~1.54 trunk reads/round), 21% per-row verify FA (fa_decode_vec_q 201us x T rows x 8 full layers — 392-CTA launch = 4.8 CTA/SM vs 12 achievable), 17% draft graphs (~58% of that = q6_K lm_head), 3.5% host gaps (loop already async-clean; no free host win).
- **Stage 1 — MULTI-ROW FUSED VERIFY FA (design A) LANDED:** `fa_decode_vec_q_rows` + `fa_decode_combine_rows`, grid.z=row, per-row t_kv_r/n_splits_r derived in-kernel from the same fa_split_keys formula ⇒ bit-identical per row (kernel-check pins rows-vs-loop bitdiff=0 incl a 64-boundary crossing). Engages when base_len+1 ≥ FA_VEC_MIN_TKV; `BW24_FA_ROWS_OFF=1` seam. Same-session A/B (clock-locked, specK3 pmin0.2): 9B p2 71.9→81.8 (0.82→0.93x), p3 63.4→67.0 (0.72→0.76x); 27B p2 37.0→42.0 (1.09→1.24x), p3 31.9→33.0 (0.94→0.97x). Fused per-row cost 140us vs 201us — floor = single-warp 64-key walk latency (split=64 exactness-pinned).

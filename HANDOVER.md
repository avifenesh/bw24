...
_Written 2026-07-03, standings updated 2026-07-07. Read this cold, then continue. bw24 = from-scratch Rust+CUDA LLM inference engine, target rig RTX 5090 Laptop (sm_120a, Blackwell consumer, 24GB, **858 GB/s measured read wall** — microbenched, not the 847 spec). Box bw24-g7e RETIRED from bw24 duty 2026-07-09 (owner): the running lane/w4a8v2 is its LAST task — after it reports, no further box use. All work local-only. Box-era lessons stand: kernel verdicts do not transfer across power walls (J/token law); fetch box branches via ssh remote. Repo PUBLIC: https://github.com/avifenesh/bw24 — both rigs sync via origin. L40S/sm_89 lane CLOSED (box terminated)._
...
SCOPE LAW (owner, 2026-07-09): THIS box only. Every target, lane, and number exists to make the local rig (RTX 5090 Laptop 24GB) faster — other machines are dev/verify surfaces, never the result. M3 specifically: its number is the LOCAL ~1.5 tok/s (NVMe capacity-bound); improving it on bigger boxes is out of scope. It stays on the roadmap only if a local lever exists (disk-tier streaming efficiency, or a lighter expert cut that genuinely fits 24GB VRAM + 60GB RAM); otherwise it is a loader/architecture capability demo, not a perf lane.

FORMAT DIRECTION (owner, 2026-07-09): support BOTH GGUF and safetensors at HIGH PERFORMANCE — squeeze both, then choose what we ourselves run. GGUF is an extra conversion step and a limitation: NOT the preferred default; ST is the native path (checkpoints ship as ST). The ST-perf work (e4m3-direct, FP8 prefill, NV-27B lane) was commissioned for exactly this; the 9B ST lands on top of the same machinery. Research rides the platform AND sets its direction — the MTP-heal protocol is the current example (its needs drove FULL_PREC + the ST tooling).
...
ROADMAP ORDER (owner, 2026-07-09): (1) QWEN IS NOT DONE — the ST path (safetensors NV-27B lane) still has improvement room: decode 92.5 spec vs the GGUF twin's class, the FP8-stash endgame (drop the Q8_0 duplicate via an e4m3-reading decode kernel = full-coverage FP8 prefill inside 24GB), and the ST plain number. Finish Qwen first. (2) THEN Gemma-4 and Hy3, before-or-parallel to each other. (3) Hy3 target = REAP50 (~150B, ~84GB @4-bit), NOT REAP75: the disk tier IS the specialty being developed — REAP50 puts the majority of experts in RAM+VRAM (good baseline perf) with a real NVMe minority to exercise and improve the spilling machinery; REAP75's model-quality drop is too steep. Blockers priced: no CUDA-format checkpoint exists (published cuts are MLX 4-bit only — transcode MLX→Q4_K keeps asymmetry, MLX→NVFP4 pays the asym→sym tax; quality gates decide), and hy_v3 is a new arch port (Hunyuan MoE; M3's sigmoid-router work is the prior).

RELEASE POLICY (owner, 2026-07-08): tag a GitHub release for every board-moving or user-facing change — minor bump per mechanism/board move, patch for fixes/docs; notes say what moved and why (public change tracking; no retirement-note verbosity — young project, state current truth plainly). First: v0.1.0. `gh release create vX.Y.Z --notes-file ...` after the merge lands.

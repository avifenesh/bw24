...
- **FA-decode port (fattn-vec structure): agent died twice on API 503/timeout — NOT DONE.** fa_decode_vec_q still dequants K/V to bf16 smem per tile with block syncs (126us vs llama 10.4us/launch). The port brief stands (q8_1 Q + dp4a on raw q8_0 K bytes, register V dequant, no block sync); re-dispatch or do inline next session. Gate battery must include run-spec K=1..4 (verify now routes through fa_decode).

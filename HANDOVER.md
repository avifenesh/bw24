...
9B plain eager 90.2 → **spec K=3 pmin=0.2: 106.6 tok/s (1.18x), all exact (9B K1-8 + 27B real-prompt K1-8)** ≈ llama plain tg128 (106.3). Landed since correction: device embed gather (HtoD was 84% of spec API time; model-lifetime resident table + embed_gather_u32_t), batched-matvec restore in matmul_decode_exact (divergence fix had over-rotated — the batched kernel is bit-identical rel=0.00e0; only dp4a's reduce shape was the hazard). Prior 47/52 prints were prime-polluted (ratios stood, absolutes didn't).

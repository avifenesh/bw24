...
- **RESULT (protocol K=3 pmin0.2, clock-locked, N=5 medians on headline cells): 9B 130.8/100.9/80.2 tok/s (1.41x/1.15x/0.91x, was 1.16/0.93/0.72 pre-work same-session); 27B 66.0/59.7/45.3 (1.86x/1.76x/1.33x — was 1.34/1.09-1.24/0.94). 27B spec now WINS at every size; 9B holds ≥1.0x through p2.** Config sweep (JSONL `spec-config-sweep`): post-replay-free the 9B optimum moved to **K=2 pmin=0.3 uniform: 137.7/104.1/88.5 = 1.48x/1.19x/1.00x — parity at 6k ctx** (was 0.72x); 27B K=2/0.2 = 1.37x at p3; 27B K=4 cliff (T=5 splits b4 MMVQ into b4+b1). Full battery green on the final tree incl all 5 seams. **NEXT levers (profiled, 9B p3 24.5ms/round): (a) MMVQ b4 efficiency — 42% of wall; ncu: batched kernels issue-stalled (10.4 active warps/sched, 1.46 eligible, DRAM 57% vs 69% for m=1-class). FIRST ATTEMPT NEGATIVE (JSONL `mmvq-b4-clamp-NEGATIVE`, reverted): removing the runtime `break` in the column unroll won b4 -15% L2-hot but did NOT transfer DRAM-bound and regressed b2 (K=2 e2e -3%) — the stall is not break-serialization; next attempt needs full ncu source counters (suspects: strided 36B-superblock weight loads x 4 activation streams, or reg-pressure occupancy). Cross-column reorder is exactness-FREE (independent accumulators). (b) draft lm_head 22% (model-bound, FR-Spec 9B negative); (c) fa_rows 17% (single-warp walk-latency floor, split=64 pinned).**
...
- **Spec-loop graph integration: draft chain DONE (6502142); verify capture CLOSED with measured proof** — see the wall-ledger row + JSONL `graph-spec-stage3-remeasure` (2026-07-03 re-measure at the image-2 optima: ceiling 2.8-3.8% of round, verify already 95-97% GPU-busy, capture cost negative through NGEN~500). Ranked 27B spec levers from that trace: (1) qmatvec_nvfp4_mmvq_b4 efficiency (63% of the whole round), (2) acceptance 0.58→0.75 (draft quality), then nothing structural.

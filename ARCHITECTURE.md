...
A from-scratch LLM inference engine for **one** machine: NVIDIA **RTX 5090 Laptop** (consumer Blackwell **GB203, sm_120**, compute capability **12.0**, **24463 MiB** VRAM, **~896 GB/s** GDDR7 [847 GB/s measured = 94.6% of peak], 82 SMs, 3090 MHz max SM clock, 64 MB L2), Intel **Core Ultra 9 275HX** (24 cores, **no AMX/AVX-512** but **has AVX2 + AVX_VNNI** int8), **elastic free host RAM** (~12–16 GB free *right now*, varies with other LLM servers — query at runtime, never hardcode), **2× WD PC SN8000S** NVMe (**PCIe Gen4 x4, ~7 GB/s each**, RAID0 ≈ 14 GB/s), GPU on **PCIe Gen5 x8** (~31 GB/s ceiling; idles at Gen1 — re-clocks under load). **Power is NOT a hard constraint:** the "150 W cap" is an asus-armoury firmware-settings bug (`nv_tgp.max_value` wrongly clamped; spec = 150 TGP + 25 boost = 175 W; user's patch to the asus-linux maintainer raises it). Real sustained limit is THERMAL (87 °C target), not a fixed wattage — benchmark at full power (`gpu-full-power on`). Toolchains present (MUTABLE — can upgrade): CUDA **13.1** + **12.8**, driver **595** (CUDA 13.2 runtime); CUTLASS/deps installable as research dictates.

...
A from-scratch LLM inference engine for **one** machine: NVIDIA **RTX 5090 Laptop** (consumer Blackwell **GB203, sm_120**, compute capability **12.0**, **24463 MiB** VRAM, **~150 W** power cap, **~896 GB/s** GDDR7), Intel **Core Ultra 9 275HX** (24 cores, **no AMX, no AVX-512**), **~12–16 GB** free host RAM (shared with other LLM servers), **2× WD PC SN8000S** NVMe (**PCIe Gen4 x4, ~7 GB/s each**), GPU on **PCIe Gen5 x8** (~31 GB/s ceiling). Toolchains present: CUDA **13.1** (default nvcc) and **12.8**, driver **595** (CUDA 13.2 runtime).

**Goal:** beat vLLM, SGLang, and llama.cpp on *this exact box* for single-stream/low-concurrency decode, MoE-that-doesn't-fit, and tokens-per-watt. Must support MoE, weight/expert/KV spilling, GGUF, KV-cache quantization. Copying a leading impl 1:1 is allowed and encouraged.
...
4. **The build toolchain is a correctness landmine.** llama.cpp MMQ **segfaults under CUDA 13.1** on sm_120 → must build the GGUF/MMQ path with **CUDA 12.8**. Arch flag must be **`sm_120a` (SASS)**; FP4-fast grouped needs **`compute_120f` (CUDA 13.0+)**. There is also a **driver-595 `sharedMemPerBlockOptin` overflow** that aborts MMQ at runtime regardless of toolkit — must port llama.cpp's smpbo clamp (PR #22338).
...
6. **Numbers in the wild are mostly desktop/datacenter.** Desktop 5090 ≈ 1792 GB/s / 575 W; **this laptop ≈ 896 GB/s / 150 W** — roughly **half**. Every published tok/s must be halved as a first approximation, and re-measured before any "beats X" claim (perf claims require **N=5 medians**).
...
| **NVFP4 *grouped*/MoE GEMM** | (research only) | **risky** (TMA-WS tactics fail even under 120f; ~39 < Marlin 46–50; silent garbage on 120a) | **Marlin W4A16 grouped (default)** |
...
| **Sampling (top-k/p/min-p, rejection)** | FlashInfer `sampling.cuh` (CUB + curand) | **confirmed** (verified on this box; audit #3170 marks arch-agnostic; ex. 0.042 ms @128k batch=1) | CPU sampler (llama.cpp), <1 ms on the 275HX |
...
**Copy from:** `vllm/.../fused_moe/{fused_moe.py, moe_align_block_size.py}` + `fused_marlin_moe.py`; `llama.cpp/ggml/src/ggml-backend.cpp:1445-1564` (selective expert copy) + **RFC #20757 / tinyserve PoC** (SLRU two-tier slot cache — *note: this is an active llama.cpp RFC, so the offload edge is time-limited*); imp single-CUDA-graph scheduler + Expert Deferral idea; SGLang `eplb/expert_distribution.py` as the hot-expert signal.
...
**Edge 2 — Block-scaled FP4/FP8 under the 150 W cap (confirmed feasible AND a real compute win, scoped).**
...
- **Beat-target:** **best tok/s-per-watt** of the four engines on 8B-Q4 at 150 W; prefill **~3–5k tok/s laptop** (≈ half desktop's 6–10k), with block-scaled FP4 prefill as the lever competitors mostly don't wire on sm_120 yet.

**Edge 3 — MoE-that-doesn't-fit (real but conditional, *demoted from "biggest win"*).**
- vLLM/SGLang OOM (0 tok/s). llama.cpp `--n-cpu-moe` already does **8–31 tok/s** on a *desktop* 5090 (the verdict refuted the "0.5–2 tok/s" strawman). Our SLRU resident cache + correct sm_120 kernels target **a ~1.5–2× beat over llama.cpp in the offload regime**, not 6–25×. With only 12–16 GB free host RAM, a 120B's experts spill to ~7 GB/s SSD, so honest laptop target is **~8–12 tok/s** for 120B-class, and the cache delivers **zero** benefit for the 30B-A3B-Q4 *primary* target (which fits 24 GB and runs fully resident at bandwidth limit — Edge 1's megakernel is the lever there).

**The moat is whole-system:** native Rust runtime + the *right copied/correct* sm_120 kernels (cuBLASLt + Marlin + vLLM dense NVFP4 + llama.cpp MMQ/fattn-vec) + this-laptop tuning (99 KB smem tiles, 82 SMs, 896 GB/s, 150 W, x8 PCIe, ~7 GB/s NVMe). **Not** a faster matmul, **not** native FP4 MoE, **not** a unique offload architecture (RFC #20757 may land upstream).
...
- Gate: 120B-class MoE runs (where vLLM/SGLang OOM); **≥1.5× llama.cpp `--n-cpu-moe`** in the offload regime; honest ~8–12 tok/s laptop target.
...
**R1 — The headline decode win is unmeasured on *this* part.** Every cited tok/s/SOL is desktop @400–575 W / 1792 GB/s or datacenter. Laptop is ~half bandwidth, 150 W power-bound (may cap below tensor SOL), and gau-nernst's 94% SOL was *non-causal, desktop*.
→ *Mitigation:* Phase-0/1 microbench mandatory before any beat-claim; recompute SOL for 896 GB/s + 82 SM + 150 W; add causal+GQA and re-profile at N=5. Keep llama.cpp as the live baseline in every benchmark.
...
**R5 — The MoE-offload edge is time-limited and smaller than hoped.** llama.cpp RFC #20757 implements the same two-tier SLRU cache; with 12–16 GB free RAM the 120B working set spills to ~7 GB/s SSD; the 30B *primary* target doesn't spill at all.
...
1. Sustained H2D bandwidth under the 150 W cap (link idles at gen1) — sets prefetch break-even. (Phase 4)

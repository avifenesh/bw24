...
| `BW24_KV_K` | `q8_0` | K-cache format arm. `fp8` FLIP-BLOCKED (2026-07-09 A/B, tag kvk-fp8-ab-9bst): e2e FLAT (+0.3-0.9% ms/tok — the −7-14% micro didn't survive; micro≠e2e) AND 9B ST spec self-consistency FAIL (acceptance 74%→20.5%, drift accumulates across K reads). q8_0 stays. `nvfp4` (2026-07-20, kv-fp4 lane): 18 B/32elem (−47% K bytes; e2m1 + per-16 e4m3 sub-scales — the NVFP4-KV recipe), self-consistency PASS all K, battery green in-config; needs per-model kf4 draft files (drafts/ on the HF bench repo) and stays a NEW NUMERIC CONFIG pending the rig depth-cell adoption call |
| `BW24_KV_V` | `q5_1` | V-cache format arm: `q4_0` FLAT + quality-taxed (argmax MISMATCH in-config), `fp8` borderline (−2pt) — measured 2026-07-08; q5_1 stays. `nvfp4` (2026-07-20): 18 B/32elem (−25% V bytes), same block layout + status as the K arm; pair both for the −38%/token KV shrink |

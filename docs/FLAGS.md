...
| `MEMRA_MOE_F16G` | ON (90a, mode 2, silu class); AUTO-KQUANT (sm_120a, mode 3) | grouped f16 expert prefill lane (round 49 promotion, mode 1): q35 board prime 5490 -> 8380 (+53%) at 41/41 dequant coverage — the last board loss flipped. Round 55 (2026-08-02, lane/h100-flip-full): Hopper default mode 1 -> 2 — the sk visitor with full direct-loader coverage + deep tail beats cublas mode 1 +52.6% (q35 board-2048 13163.6 vs 8626.5, x5 interleaved, argmax 30/30). Round 50: the gemma-MoE (gelu) site defaults OFF per model (g26 board-2048 prefill -8.3% under the round-49 default; `=1`/`=2` still opens it). sm_120a AUTO-KQUANT default (2026-08-02, lane/q4k-expert-prefill): the mode-2 sk form admitted only for k-quant expert layers the int8-MMA MMQ arm rejects (their baseline was the per-pair `_em` fallback) — Ornith-35B Q4_K board-2048 1098 -> 3454 (3.14x); IQ-bank layers keep MMQ (the round-49 FLAT verdict holds for those). Modes + full history in §5; `=0` reverts |

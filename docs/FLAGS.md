...
| `MEMRA_SERVE_SPEC` | on (GGUF); OFF for dir ckpts | `0` disables the spec-decode serve path (greedy + MTP-head requests). DIR checkpoints (safetensors/repack) are spec-QUARANTINED by default — explicit `MEMRA_SERVE_SPEC=1` forces the experimental door (serve spec-session diverges from plain greedy on ST models while run-spec CLI passes K=1..8; receipts `research/serve-st-20260803/RESULTS.md`, root cause open) |

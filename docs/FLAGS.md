...
| `BW24_KV_FP8=1` | qwen/non-gemma trunk KV in e4m3 (the gemma depth lever ported; draft scratch stays q8_0/q5_1, prime forced monolithic) | BRING-UP: correctness green (run-gen MATCH, run-spec K=1..6 PASS, 2026-07-12) but hd128 attention rides the g-module SCALAR at t_kv>=96 — perf verdict waits on the register/v2 g-lanes + the acceptance battery; supersedes the 2026-07-09 `BW24_KV_K=fp8` block (that collapse was verify-side plumbing, since fixed) |

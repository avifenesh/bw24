...
| `MEMRA_MMQ=1` (+`MEMRA_RP=0`) | native W4A4 FP4 MMQ prefill. Uncorrected (`MEMRA_MMQ_RESIDUAL_K=0`) 1.62x our default; with the k=32 residual correction 1.34x (q9 pp1845, interleaved x5, 2026-08-03) | EXACTNESS BLOCK LIFTED **only** with `MEMRA_MMQ_RESIDUAL_K=32` (2026-08-03, lane/w4a4-rescue, receipts `research/w4a4-rescue-20260803/`): 5/5 measurable gate cells token-IDENTICAL 48/48 (q9+q27 x p2/p3/p4-16k), reproduced across four kernel revisions; battery green under the arm — kernel-check ALL GREEN, run-gen argmax MATCH q9+q27, run-spec K=1..8 PASS both models (`logs/battery-k32.log`). **k=0 stays EXACTNESS-BLOCKED** — the bare e2m1 activation grid still forks argmax on long real prompts, which is the whole reason the correction exists. Not yet a default: see §5 note below |
| `MEMRA_MMQ_RESIDUAL_K=<k>` (default 0) | residual outlier channels kept in f32 alongside the W4A4 path: the k largest-amax activation channels are zeroed before quantization and their exact contribution is added back as a rank-k update. Clamped to 64, rounded up to a compile-time bucket (8/16/32/64) whose padding lanes are numeric no-ops | **32 is the exactness point, and it is a plateau — not a monotone trend.** Token-identical cells out of the 5 measurable (`residual-k*.jsonl`): k=4 → 3/5, k=8 → 2/5, k=16 → 3/5, **k=32 → 5/5** (reproduced across 4 kernel revisions), k=64 → 0/5. Do not assume "larger k is safer". Costs 1.62x → 1.34x of the uncorrected arm. Read live per call, since the gate sweeps arms inside one process |

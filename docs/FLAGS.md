...
| `MEMRA_SERVE_TOKDEFER` | ON | `0` = per-chunk [B]-u32 device-token readback + stream sync inside every batched step (the pre-inc3 tick). Default: all chunks of a tick write their device-sampled tokens into ONE shared device buffer and the worker D2Hs it once after the last chunk — one D2H+sync per tick instead of one per chunk, and chunk launches pipeline instead of serializing on the readback. Token values bit-unchanged (same argmax/gumbel launches, different output slot); serving arbiter: greedy-hash + check-batch-exact (`research/batched-tick-inc3-20260801/`) |

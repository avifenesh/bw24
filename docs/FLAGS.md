...
| `MEMRA_PP_STAGES=2` / `MEMRA_PP_SPLIT=<layer>` | pipeline-parallel 2-stage seam (M1 of the multi-GPU build): decode_step runs layers [0,split) as stage 0, hands the [n_embd] f32 residual through a dedicated boundary buffer (two real dtod copies — never an alias), runs [split,n) as stage 1. Split defaults n_layers/2. Generic eager + gemma4 arms only (batch/dc/graph/spec warn-once) | M1 increment 1 (2026-08-01): BIT-IDENTICAL via pp2-gate — q9 q8_0 splits 16/5/31 + g12 q4_0 splits 24/7, 48 steps each, full-logit bit compare (`research/m1-pp2-20260801/`). Single-device today; increment 2 swaps the buffer middle for cudaMemcpyPeerAsync (M0-measured 2.8x NCCL at PP sizes) + per-stage streams. Default OFF, zero behavior change unset |
